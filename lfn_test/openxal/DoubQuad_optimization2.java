...
public class DoubQuad_optimization2 {
...
//		1. 加载加速器配置
		...
		String seqname = "CLAPA1";
		AcceleratorSeqCombo Acc_Seq = clapa1.getComboSequence(seqname);
...
//			Map<String, Double> received = getstring	
			...
			G1 = params.getDouble("G4");
			G2 = params.getDouble("G5");
//			K0 = params.getDouble("K0");
			
			QV04.setDfltField(G4);
			QH05.setDfltField(G5);
...
//			sql = String.format("UPDATE bayes_optimization SET sigmaX=%f, sigmaY=%f, finished=%d WHERE id=%d", sigma[0], sigma[1], 1, rs.getInt("id"));
			sql = String.format("INSERT INTO bayes_optimization_DoubQuad(G4, G5, sigmaX, sigmaY, finished) VALUES(%f, %f, %f, %f, %d)", G4, G5, sigma[0], sigma[1], 1);
